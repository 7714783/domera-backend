...
export class TakeoverService {
  ...
  async createCase(
    tenantId: string,
    actorUserId: string,
    body: { buildingSlug: string; outgoingOrgSeedKey?: string; incomingOrgSeedKey?: string; targetGoLiveAt?: string },
  ) {
    const building = await this.prisma.building.findFirst({ where: { tenantId, slug: body.buildingSlug } });
    if (!building) throw new NotFoundException('building not found');

    const outgoing = body.outgoingOrgSeedKey
      ? await this.prisma.organization.findFirst({ where: { tenantId, seedKey: body.outgoingOrgSeedKey } })
      : null;
    const incoming = body.incomingOrgSeedKey
      ? await this.prisma.organization.findFirst({ where: { tenantId, seedKey: body.incomingOrgSeedKey } })
      : null;
  ...
  async gapAnalysis(tenantId: string, caseId: string) {
    ...
    for (const t of statutoryTemplates) {
      const applicable = t.applicabilityRules.length === 0 || t.applicabilityRules.every((r) => this.evalPredicate(r.predicate as any, ctx));
    ...
    const tasks = await this.prisma.taskInstance.findMany({
      ...
      select: { id: true, title: true, status: true, dueAt: true, evidenceRequired: true, evidenceDocuments: true, requiredCertificationKey: true, completedByUserId: true },
    });
    const overdueCompletions = tasks.filter((t) => t.status === 'overdue');
    const missingEvidence = tasks.filter(
      (t) => t.evidenceRequired && (!t.evidenceDocuments || (Array.isArray(t.evidenceDocuments) && (t.evidenceDocuments as any[]).length === 0)) && t.status !== 'open',
    );
    ...
    const qualificationGaps = tasks
      .filter((t) => t.requiredCertificationKey && !availableCertKeys.has(t.requiredCertificationKey))
      .map((t) => ({ id: t.id, title: t.title, requiredCertificationKey: t.requiredCertificationKey }));
    ...
    return {
      ...
      overdue_completions: overdueCompletions.map((t) => ({ id: t.id, title: t.title, dueAt: t.dueAt.toISOString() })),
  ...
  async signoff(tenantId: string, caseId: string, actorUserId: string, actorRole: string) {
    const c = await this.prisma.takeoverCase.findFirst({ where: { id: caseId, tenantId } });
    if (!c) throw new NotFoundException('takeover case not found');
    if (!SIGNOFF_CHAIN.includes(actorRole)) throw new ForbiddenException('role cannot sign off takeover');
    ...
    if (expectedNext !== actorRole) throw new BadRequestException(`signoff order violated: expected ${expectedNext}, got ${actorRole}`);
  ...
  private evalPredicate(predicate: { attr: string; op: string; value: number | string }, ctx: Record<string, any>): boolean {
  ...
  private async buildingContext(buildingId: string): Promise<Record<string, number | string | null>> {
