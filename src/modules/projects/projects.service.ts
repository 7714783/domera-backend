...
const CO_STATUSES = ['pending', 'approved', 'rejected', 'superseded'];
const ACCEPTANCE_STATUSES = ['draft', 'submitted', 'partially_signed', 'accepted', 'rejected'];
...
export class ProjectsService {
  ...
  async getProject(tenantId: string, id: string) {
    ...
    const [stages, budgetLines, changeOrders, acceptance] = await Promise.all([
      ...
      this.prisma.projectBudgetLine.findMany({ where: { projectId: id }, orderBy: { createdAt: 'asc' } }),
      this.prisma.changeOrder.findMany({ where: { projectId: id }, orderBy: { createdAt: 'desc' } }),
  ...
  private rollup(lines: any[], cos: any[]) {
    const plannedCapex = lines.filter((l) => l.classification === 'capex').reduce((a, b) => a + b.plannedAmount, 0);
    const plannedOpex = lines.filter((l) => l.classification === 'opex').reduce((a, b) => a + b.plannedAmount, 0);
    const actualCapex = lines.filter((l) => l.classification === 'capex').reduce((a, b) => a + b.actualAmount, 0);
    const actualOpex = lines.filter((l) => l.classification === 'opex').reduce((a, b) => a + b.actualAmount, 0);
    const approvedCos = cos.filter((c) => c.status === 'approved');
    const coCapex = approvedCos.filter((c) => c.classification === 'capex').reduce((a, b) => a + b.costDelta, 0);
    const coOpex = approvedCos.filter((c) => c.classification === 'opex').reduce((a, b) => a + b.costDelta, 0);
    return {
      plannedCapex, plannedOpex, actualCapex, actualOpex,
      coCapex, coOpex,
  ...
  async createProject(tenantId: string, actorUserId: string, body: {
    buildingId: string; name: string; stage?: string;
    capexBudgetId?: string; recommendationId?: string;
    ...
    const project = await this.prisma.project.create({
      data: {
        ...
        stageHistory: [{ stage: body.stage || 'initiation', at: new Date().toISOString(), by: actorUserId }] as any,
      },
    });
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'project_owner',
      action: 'project.created', entity: project.id, entityType: 'project',
      building: project.buildingId, ip: '-', sensitive: false,
    });
    return project;
  }

  async classify(tenantId: string, actorUserId: string, id: string, body: { classification: string; reason?: string }) {
    ...
    history.push({
      from: p.classification, to: body.classification,
      reason: body.reason || null, at: new Date().toISOString(), by: actorUserId,
    });
    ...
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'finance_controller',
      action: `project.classify.${p.classification}_to_${body.classification}`,
      entity: id, entityType: 'project',
      building: p.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }
  ...
  async advanceStage(tenantId: string, actorUserId: string, id: string, nextStage: string) {
    ...
    const updated = await this.prisma.project.update({
      where: { id },
      data: { stage: nextStage, stageHistory: history as any, closedAt: nextStage === 'closed' ? new Date() : null },
    });
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'project_owner',
      action: `project.stage.${p.stage}_to_${nextStage}`, entity: id, entityType: 'project',
      building: p.buildingId, ip: '-', sensitive: false,
    });
    return updated;
  }
  ...
  async createStage(tenantId: string, projectId: string, body: any) {
    ...
    if (!p) throw new NotFoundException('project not found');
    if (!body.name || body.orderNo === undefined) throw new BadRequestException('name + orderNo required');
    ...
    return this.prisma.projectStage.create({
      data: {
        tenantId, buildingId: p.buildingId, projectId,
        orderNo: body.orderNo, name: body.name,
  ...
  async addBudgetLine(tenantId: string, projectId: string, body: any) {
    ...
    return this.prisma.projectBudgetLine.create({
      data: {
        ...
        plannedAmount: body.plannedAmount, actualAmount: body.actualAmount || 0,
  ...
  async convertLineClassification(tenantId: string, actorUserId: string, lineId: string, to: string, reason?: string) {
    ...
    const updated = await this.prisma.projectBudgetLine.update({
      where: { id: lineId },
      data: { classification: to, notes: [line.notes, `[${new Date().toISOString()}] classification ${line.classification}→${to}${reason ? `: ${reason}` : ''}`].filter(Boolean).join('\n') },
    });
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'finance_controller',
      action: `budget_line.convert.${line.classification}_to_${to}`,
      entity: lineId, entityType: 'project_budget_line',
      building: line.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }
  ...
  async createChangeOrder(tenantId: string, actorUserId: string, projectId: string, body: any) {
    ...
    return this.prisma.changeOrder.create({
      data: {
        tenantId, buildingId: p.buildingId, projectId,
        coNumber: body.coNumber, title: body.title,
        description: body.description || null,
        scopeDelta: body.scopeDelta || null,
        costDelta: body.costDelta, currency: body.currency || 'ILS',
  ...
  async decideChangeOrder(tenantId: string, actorUserId: string, coId: string, decision: 'approved' | 'rejected') {
    if (!['approved', 'rejected'].includes(decision)) throw new BadRequestException('invalid decision');
    ...
    if (co.status !== 'pending') throw new BadRequestException(`cannot decide on CO in status ${co.status}`);
    ...
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'project_owner',
      action: `change_order.${decision}`, entity: coId, entityType: 'change_order',
      building: co.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }

  // ── Acceptance pack / handover ─────────────────────────
  async upsertAcceptancePack(tenantId: string, projectId: string, body: {
  ...
  async submitAcceptancePack(tenantId: string, actorUserId: string, projectId: string) {
    ...
    if (pack.status !== 'draft') throw new BadRequestException(`cannot submit pack in status ${pack.status}`);
    ...
    if (required.length > 0) {
      ...
      const attached = new Set(attachedDocs.map((d) => d.documentTypeKey).filter(Boolean) as string[]);
    ...
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'project_owner',
      action: 'acceptance_pack.submitted', entity: pack.id, entityType: 'acceptance_pack',
      building: pack.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }

  async signoffAcceptancePack(
    tenantId: string, actorUserId: string, projectId: string,
    ...
    const conflict = Object.entries(existingSignoffs).find(([role, uid]) => role !== signoff && uid === actorUserId);
    if (conflict) {
      throw new ForbiddenException(`SoD: user already signed as ${conflict[0]}; cannot also sign as ${signoff}`);
    }

    const data: any = {};
    const now = new Date();
    if (signoff === 'contractor') { data.contractorSignoffByUserId = actorUserId; data.contractorSignoffAt = now; }
    if (signoff === 'manager') { data.managerSignoffByUserId = actorUserId; data.managerSignoffAt = now; }
    if (signoff === 'chief_engineer') { data.chiefEngineerSignoffByUserId = actorUserId; data.chiefEngineerSignoffAt = now; }
    if (signoff === 'owner') { data.ownerSignoffByUserId = actorUserId; data.ownerSignoffAt = now; }
    ...
    await this.audit.write({
      tenantId, actor: actorUserId, role: `acceptance_${signoff}`,
      action: `acceptance_pack.signoff.${signoff}`, entity: pack.id, entityType: 'acceptance_pack',
      building: pack.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }

  async rejectAcceptancePack(tenantId: string, actorUserId: string, projectId: string, reason: string) {
    const pack = await this.prisma.acceptancePack.findFirst({ where: { projectId, tenantId } });
    if (!pack) throw new NotFoundException('acceptance pack not found');
    if (!['submitted', 'partially_signed'].includes(pack.status)) {
    ...
    await this.audit.write({
      tenantId, actor: actorUserId, role: 'project_owner',
      action: 'acceptance_pack.rejected', entity: pack.id, entityType: 'acceptance_pack',
      building: pack.buildingId, ip: '-', sensitive: true,
    });
    return updated;
  }
