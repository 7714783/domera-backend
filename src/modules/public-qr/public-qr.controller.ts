...
export class PublicQrController {
  ...
  async landing(@Param('qrId') qrId: string, @Req() req: Request) {
    ...
    const rl = rateLimit({
      key: `qr:landing:${ip}`,
      windowMs: LANDING_WINDOW_MS,
      max: LANDING_MAX_PER_IP,
    });
    if (!rl.allowed)
  ...
  async submit(@Param('qrId') qrId: string, @Body() body: any, @Req() req: Request) {
    ...
    const ipRl = rateLimit({
      ...
      max: SUBMIT_MAX_PER_IP,
    });
    if (!ipRl.allowed)
      throw new HttpException(
        { error: 'rate limited (ip)', retryAfterMs: ipRl.retryAfterMs },
        HttpStatus.TOO_MANY_REQUESTS,
      );
    const qrRl = rateLimit({
      key: `qr:submit:qr:${qrId}`,
      windowMs: SUBMIT_WINDOW_MS,
      max: SUBMIT_MAX_PER_QR,
    });
    if (!qrRl.allowed)
      throw new HttpException(
        { error: 'rate limited (qr)', retryAfterMs: qrRl.retryAfterMs },
        HttpStatus.TOO_MANY_REQUESTS,
      );
  ...
  async png(
    ...
    if (!rl.allowed)
      throw new HttpException({ error: 'rate limited' }, HttpStatus.TOO_MANY_REQUESTS);
    const buf = await this.qr.qrPngBuffer(
      qrId,
      base || baseUrlFromHost(req, process.env.APP_PUBLIC_BASE_URL),
    );
