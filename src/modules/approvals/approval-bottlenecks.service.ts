...
export class ApprovalBottlenecksService {
  ...
  async snapshot(tenantId: string, params: { buildingId?: string; defaultSlaHours?: number } = {}) {
    ...
    const byRole = new Map<
      string,
      { count: number; waits: number[]; buildings: Set<string>; thresholds: Set<string> }
    >();
    const byThreshold = new Map<string, { count: number; waits: number[] }>();
    const now = Date.now();
    const oldestPending: Array<{
      requestId: string;
      building: string;
      role: string;
    ...
    for (const r of requests) {
      ...
      const rAcc = byRole.get(pending.role) ?? {
        count: 0,
        waits: [],
        buildings: new Set(),
        thresholds: new Set(),
    ...
    const thresholdRows = [...byThreshold.entries()]
      .map(([threshold, acc]) => {
