...
export class CleaningAccessService {
  ...
  async resolve(tenantId: string, userId: string): Promise<CleaningActor> {
    const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { id: true, isSuperAdmin: true } });
    if (!user) throw new UnauthorizedException('user not found');
    if (user.isSuperAdmin) return { kind: 'platform_admin', userId, tenantId };

    const ws = await this.prisma.membership.findFirst({
      where: { tenantId, userId, roleKey: { in: ['workspace_owner', 'workspace_admin', 'org_admin'] } },
    });
    if (ws) {
      const buildings = await this.prisma.building.findMany({ where: { tenantId }, select: { id: true } });
      return { kind: 'operations_manager', userId, tenantId, buildingIds: buildings.map((b) => b.id) };
    }

    const bmRoles = await this.prisma.buildingRoleAssignment.findMany({
      where: { tenantId, userId, roleKey: { in: ['building_manager', 'operations_manager', 'chief_engineer'] } },
      select: { buildingId: true },
    });
    if (bmRoles.length > 0) {
      return { kind: 'building_manager', userId, tenantId, buildingIds: bmRoles.map((r) => r.buildingId) };
    }
    ...
    const contractor = await this.prisma.cleaningContractor.findUnique({ where: { id: staff.contractorId } });
    if (!role || !contractor) throw new ForbiddenException('cleaning role/contractor missing');

    switch (role.code) {
      case 'boss':
        return { kind: 'cleaning_boss', userId, tenantId, contractorIds: [contractor.id], buildingId: contractor.buildingId };
      case 'manager':
        return { kind: 'cleaning_manager', userId, tenantId, contractorIds: [contractor.id], buildingId: contractor.buildingId };
      case 'supervisor': {
        ...
        return {
          kind: 'cleaning_supervisor', userId, tenantId,
          staffId: staff.id, contractorId: contractor.id,
      ...
      case 'dispatcher':
      default:
        return { kind: 'cleaner', userId, tenantId, staffId: staff.id, contractorId: contractor.id };
    }
  }
}
