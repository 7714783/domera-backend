...
export class IamService {
  ...
  async canDelegate(actorUserId: string, buildingId: string, targetRoleKey: string): Promise<boolean> {
  ...
  async assign(
    ...
    const existing = await this.prisma.buildingRoleAssignment.findUnique({
      where: { buildingId_userId_roleKey: { buildingId, userId: body.userId, roleKey: body.roleKey } },
    });

    const result = existing
      ? await this.prisma.buildingRoleAssignment.update({
          where: { id: existing.id },
          data: { expiresAt: body.expiresAt ? new Date(body.expiresAt) : null, delegatedBy: actorUserId, delegatedAt: new Date() },
        })
  ...
  async createStaff(
    ...
    if (!body.displayName || !body.roleKey) throw new BadRequestException('displayName and roleKey required');
    ...
    if (!canDelegate && !actorMembership) throw new ForbiddenException('actor cannot assign this role');

    const baseEmail = (body.email || `${body.displayName.trim().toLowerCase().replace(/[^\w]+/g, '.')}@staff.local`).toLowerCase();
    const existingEmail = await this.prisma.user.findUnique({ where: { emailNormalized: baseEmail } });
    ...
    const user = await this.prisma.user.create({
      data: {
        email: baseEmail, emailNormalized: baseEmail,
    ...
    const assignment = await this.prisma.buildingRoleAssignment.create({
      data: {
        tenantId, buildingId, userId: user.id, roleKey: body.roleKey,
        delegatedBy: actorUserId,
      },
    });
  ...
  async listBuildingStaff(tenantId: string, buildingId: string) {
    const assignments = await this.prisma.buildingRoleAssignment.findMany({
      ...
      include: {
        ...
        user: {
          select: {
            id: true, username: true, email: true, displayName: true, isSuperAdmin: true, lastLoginAt: true,
    ...
    for (const a of assignments) {
      ...
      const cur = grouped.get(key) || { orgName: org?.name || 'Unaffiliated', orgType: org?.type || null, members: [] };
      cur.members.push({
        assignmentId: a.id,
        userId: a.user.id,
        ...
        certifications: a.user.certifications.map((c) => ({ key: c.certification.key, name: c.certification.name, expiresAt: c.expiresAt })),
      });
      grouped.set(key, cur);
    }
    return {
  ...
  async revoke(tenantId: string, buildingId: string, actorUserId: string, assignmentId: string) {
    const existing = await this.prisma.buildingRoleAssignment.findFirst({ where: { id: assignmentId, tenantId, buildingId } });
