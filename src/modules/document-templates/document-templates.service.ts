...
export class DocumentTemplatesService {
  ...
  async list(tenantId: string, params: { buildingId?: string; kind?: string; includeInactive?: boolean } = {}) {
  ...
  async create(tenantId: string, actorUserId: string, body: {
    ...
    buildingId?: string; documentTypeKey?: string;
    bodyMarkdown?: string; sampleDocumentId?: string;
    requiresPhoto?: boolean; requiresDigitalSignoff?: boolean; retentionYears?: number;
  }) {
    if (!body.key || !body.name || !body.kind) throw new BadRequestException('key, name, kind required');
    ...
    if ((body.kind === 'printable' || body.kind === 'reference') && !body.sampleDocumentId) {
      throw new BadRequestException(`${body.kind} requires sampleDocumentId (upload a Document first)`);
    }
    if (body.sampleDocumentId) {
      const sample = await this.prisma.document.findFirst({ where: { id: body.sampleDocumentId, tenantId } });
    ...
    if (existing) throw new BadRequestException(`template with key "${body.key}" already exists — pick another`);
  ...
  async appliedTo(tenantId: string, id: string) {
    ...
    return {
      ...
      rounds: waypoints.map((w) => ({
        roundId: w.round.id, roundName: w.round.name, buildingId: w.round.buildingId,
        waypointId: w.id, waypointLabel: w.label,
      })),
  ...
  async update(tenantId: string, id: string, body: Partial<{
    name: string; description: string; kind: string; documentTypeKey: string;
    bodyMarkdown: string; sampleDocumentId: string | null;
    requiresPhoto: boolean; requiresDigitalSignoff: boolean; retentionYears: number | null;
    isActive: boolean; buildingId: string | null;
  }>) {
    const existing = await this.prisma.documentTemplate.findFirst({ where: { id, tenantId } });
    if (!existing) throw new NotFoundException('template not found');
    if (body.kind && !KINDS.includes(body.kind)) throw new BadRequestException(`kind must be one of ${KINDS.join(', ')}`);
  ...
  async delete(tenantId: string, id: string) {
    ...
    const inUse = await this.prisma.ppmTemplate.count({ where: { evidenceDocumentTemplateId: id } });
    if (inUse > 0) {
      throw new ForbiddenException(`template is referenced by ${inUse} PPM program(s); deactivate instead`);
    }
