 ...
 * `nextDueAt - N days` (± 12h window), queues an in-app notification
 * for:
 *   - the assignedUserId of the plan item (if set)
 *   - every `evidenceRecipient` with a `role` that maps to a building role assignment
 *
 * Notifications are idempotent per (planItemId, dueDate, N) via a
 * deterministic content tag stored in `type` — re-runs on the same day
 * do nothing new. The actual write goes through
 * `NotificationsService.recordInAppNotification(...)` so the
 * `notification` table stays single-owner under the notifications
 * module (INIT-010 Follow-up B).
 */
export async function scanAndSendReminders(
  ...
  const plans = await prisma.ppmPlanItem.findMany({
    include: {
      template: {
        select: {
          slaReminderDays: true,
          evidenceRecipients: true,
          assignedUserId: true,
          name: true,
        },
      },
  ...
  for (const plan of plans) {
    ...
    for (const n of reminderDays) {
      ...
      const already = await prisma.notification.findFirst({
        where: { tenantId: plan.tenantId, type },
      });
      if (already) continue;
      ...
      const recipients = Array.isArray(plan.template.evidenceRecipients)
        ? (plan.template.evidenceRecipients as Array<{ role?: string; userId?: string }>)
        : [];
      ...
      for (const userId of recipientUserIds) {
        await notifications.recordInAppNotification({
...
export function createSlaReminderInfra(prisma: PrismaClient, notifications: NotificationsService) {
  ...
  queue
    .upsertJobScheduler('daily-sla-scan', { pattern: '0 30 5 * * *' }, { name: JOB_NAME, data: {} })
  ...
  return { queue, worker, runNow: () => scanAndSendReminders(prisma, notifications) };
}
