...
async function upsertContractor(tenantId, buildingId, name, legalName) {
  ...
  const c = await prisma.cleaningContractor.create({
    data: {
      ...
      email: `ops@${name.toLowerCase().replace(/[^a-z]/g, '')}.example`,
    },
  });
  for (const code of ROLES) {
    await prisma.cleaningRole.create({
      data: { tenantId, contractorId: c.id, code, name: ROLE_NAMES[code] },
    });
  }
  return c;
}
...
async function run() {
  ...
  if (!building)
    throw new Error('building menivim-kfar-saba not found — seed the test building first');
  const tenantId = building.tenantId;

  const contractorA = await upsertContractor(
    tenantId,
    building.id,
    'CleanPro A',
    'CleanPro A Ltd.',
  );
  const contractorB = await upsertContractor(
    tenantId,
    building.id,
    'SparkleCo B',
    'SparkleCo B Ltd.',
  ...
  const supervisorA = await upsertStaff(
    tenantId,
    contractorA,
    'supervisor',
    'Avi Supervisor',
  ...
  const cleanerA = await upsertStaff(
    tenantId,
    contractorA,
    'cleaner',
    'Adam Cleaner',
  ...
  const cleanerB = await upsertStaff(
    tenantId,
    contractorB,
    'cleaner',
    'Boris Cleaner',
  ...
  const exists = await prisma.cleaningRequest.findFirst({
    where: { zoneId: wc3.id, title: 'Initial paper roll restock' },
  });
  if (!exists) {
    await prisma.cleaningRequest.create({
      data: {
        ...
        category: 'regular_cleaning',
        priority: 'normal',
        source: 'admin',
        contractorId: contractorA.id,
        assignedStaffId: supervisorA.id,
        status: 'assigned',
        assignedAt: new Date(),
      },
    });
    ...
    JSON.stringify(
      {
        ...
        contractors: [
          { id: contractorA.id, name: contractorA.name },
          { id: contractorB.id, name: contractorB.name },
        ],
        staff: {
          A: {
            bossA: bossA.id,
            managerA: managerA.id,
            supervisorA: supervisorA.id,
            cleanerA: cleanerA.id,
          },
